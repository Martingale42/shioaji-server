...
- 00981A point-in-time **top-300 market-cap constituent universe**
  (survivorship-correct): a cached TWSE/TPEx/MOPS official-data client
  (`scripts/twse_tpex_market.py`), point-in-time daily-share reconstruction +
  market-cap ranking + union/membership builder (`scripts/universe_ranking.py`,
  `scripts/build_00981a_universe.py`), and a 0050-style resume cron
  (`scripts/resume_00981a_fetch.sh`) — producing the 395-code
  `universe/00981a_top300_constituents.txt` and interval
  `universe/membership_00981a_top300.csv`. The members' 1-min bar download
  (BL-9) is quota-coordinated and pending sign-off.
